# Nautlis passive asset collector (c) 2016 phreaklets mini Kraken :-)
import logging import logging.handlers as handlers
# turn off those irritating IPv6 warning messages
logging.getLogger("scapy.runtime").setLevel(logging.ERROR) from contextlib import contextmanager from threading import Thread from Queue import Queue, Empty from scapy.all import load_module,Ether,IP,TCP,sniff #Import needed 
modules from scapy import sys import sqlite3 import datetime import dns.resolver from dns import reversename from netaddr import IPAddress import netaddr import os import time import getopt
#from scapy.layers import http
dbconn = None m_iface = "eth1" m_finished = False conn = 0 class SizedTimedRotatingFileHandler(handlers.TimedRotatingFileHandler):
    """
    ...
    def __init__(self, filename, mode='a', maxBytes=0, backupCount=0, encoding=None,
        ...
        # If rotation/rollover is wanted, it doesn't make sense to use another mode. If for example 'w' were specified, then if there were multiple runs of the calling application, the logs from previous runs would be lost if the 
        # 'w' is respected, because the log file would be truncated on each run.
        if maxBytes > 0:
            mode = 'a'
...
def log_setup(log_file=None):
    ...
    logger.setLevel(logging.INFO) class DbConnector:
    conn = None
    @contextmanager
    def get_cursor(self):
    ...
    def close_conn(self):
        self.conn.close() def getttl(dbconn, ttl, ip, ethsrc):
    # Get TTL
    if ttl < 64 and ttl > 49:
        ...
        dbconn.addttl(ethsrc, "Windows") def threaded_sniff_target(q):
    global m_finished
    sniff(iface = m_iface, count = 0, filter = "tcp", prn = lambda x : q.put(x))
    m_finished = True def threaded_sniff():
    load_module("p0f")
    q = Queue()
    ...
    threaded_sniff() if __name__ == "__main__":
    main(sys.argv[1:])
